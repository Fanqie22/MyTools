function checkuserMail() {
	...
	if (strValue == '' || strValue.length == 0 || strValue == null) {
		span.innerHTML = " 注 册 的 邮 箱 号 不 能 为 空 哦 !";
		...
		$.ajax({
			...
			success : function(data) {
				...
				var strValue = userInput.value;
				if (data == "0") {
					span.innerHTML = " 这 个 邮 箱 号 已 经 注 册 啦 !";
					span.style.color = "red";
					return false;
				} else {
					span.innerHTML = "";
		...
		span.innerHTML = " 这 不 是 有 效 的 邮 箱 号 哦 !";
		span.style.color = "red";
		return false;
	}
}
...
function checkregisterpassword() {
	...
	if (strValue == null || strValue.length == 0) {
		...
		span.innerHTML = " 密 码 的 位 数 应 为 5-16 位 数 哦 !";
		span.style.color = "red";
		return false;
	}
}
...
function checkregisterRepassword() {
	...
	if (strValue == RestrValue) {
		...
		span.innerHTML = " 两 次 输 入  的 密 码 不 一 致 !";
		span.style.color = "red";
		return false;
	}
}
...
function sendmail() {
	...
	if (checkuserMail() && checkregisterpassword() && checkregisterRepassword()) {
		...
		span.innerHTML = "前 面 注 册 填 写 的 信 息 为 空 或 者出 错  !";
		span.style.color = "red";
		return false;
	}
}
...
function mailcodecheck() {
	...
	if (strValue == finalcode) {
		...
		span.innerHTML = " 输 入 的 验 证 码 为 空 或 者 错 误  !";
		span.style.color = "red";
		return false;
	}
}
...
function checkregisterfrom() {
	return checkuserMail() && checkregisterpassword()
			&& checkregisterRepassword() && mailcodecheck();
}

// function checkuserPhone() {
// //获取当前用户名的输入的value
// var regPhone = /^1\d{10}$/;
// var userInput = document.getElementById("registerPhone");
// var strValue = userInput.value;
// var span = document.getElementById("register-phone-messages");
// if(strValue == '' || strValue.length == 0 || strValue == null) {
// span.innerHTML = " &nbsp; &nbsp; &nbsp; &nbsp;注 册 的 手 机 号 不 能 为 空 哦 !";
// span.style.color = "red";
// return false;
// } else if(regPhone.test(strValue)) {
// span.innerHTML = "";
// // span.style.color = "green";
// return true;
// } else {
// span.innerHTML = " 这 不 是 有 效 的 手 机 号 哦 !";
// span.style.color = "red";
// return false;
// }
//
